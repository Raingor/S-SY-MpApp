...
├── data/
│   ├── attractions.js         # 景点数据库（6城13景点：亮点/参观指南/深度讲解，与网页端 site-data.json 结构对齐）
│   └── itineraries.js         # 行程数据库（4条参考行程 + 定制行程样例，行程条目经 attractionIds 关联景点）
├── pages/
│   ├── index/                 # P1 首页（含 Richard 名人导游推荐卡，路线卡跳参考行程详情）
│   ├── customize/             # P2 行程资讯咨询与交互式问卷（含定制行程链接说明卡与样例入口）
│   ├── guide/                 # P4 Richard 李名人导游介绍与预约页
│   ├── vehicle/               # 服务3 在地用车资源对接咨询
│   ├── knowledge/             # 服务4 景点区（精选城市 → 景点列表，博物旅人风格）
│   ├── business/              # 服务5 希腊商旅随行咨询
│   ├── itinerary/             # 行程页
│   │   ├── index/             # 参考行程列表（页面直接展示的简单版）
│   │   └── detail/            # 行程详情（type=reference 简单版 / type=custom 定制详细版，分享即行程链接）
│   ├── attraction/            # 景点详情（亮点 / 参观指南 / 深度文史讲解）
│   │   └── detail/
│   └── profile/               # P3 我的页面与资料详情
...
- 参考博物旅人 App 的内容结构：精选城市（6城横滑）→ 城市下景点列表（封面图+名称+分类+简介）。
- 点击景点进入 `pages/attraction/detail`：中英文名、必看亮点（看点名+一句话讲解，支持展开全部）、参观指南（开放时间/门票/交通/值得一去/馆内服务/亲子/FAQ 等 12 键可扩展）、深度文史讲解（免费预览 + 付费解锁占位）。
- 页尾关联「包含这些景点的参考行程」，形成景点 ⇄ 行程双向导流。

### 行程页 `pages/itinerary/*`
- **参考行程**（`type=reference`，页面直接展示）：4 条甄选路线的简版逐日日程，首页路线卡直达。
- **定制行程**（`type=custom`，发客户的专属链接）：参数对齐《希腊定制旅程表》PDF——表头（行程日期/订单编号/旅客人数/语种需求/计划车型/推荐司导/服务费总额）、逐日时段（上午/中午/下午/晚上）、服务标记（接送/陪同/讲解/酒店/门票/机票/船票 ★）、夜宿城市、服务须知四组（服务须知/免费服务/出行提醒/紧急联系方式）。
- 行程条目通过 `attractionIds` 关联景点，详情页可点击景点名跳转景点详情。
- 分享卡片即行程链接：`/pages/itinerary/detail?id=<订单号>`，转发给同行家人直接打开。
- 首份定制行程样例：李女士 PT202610-08（2026.10.05–10.10，雅典+圣托里尼 6 天，€5,188）。
...
- 四类擅长方向、匿名客户评价、2026 年 9 月至 2027 年 2 月可切换预约状态日历。
...
- 服务端需确认 `/api/leads` 的字段校验、鉴权与生产返回格式（应返回 JSON 或 204），并在微信后台配置 request 合法域名；小程序会拒绝把 HTML fallback 误判为成功。
