...
assert(detail.includes('nodes="{{item.richNodes}}"'));
...
for (const locale of ['zh-CN', 'zh-TW', 'en']) {
  ...
  assert.equal(rendered.customSections.length, 2);
  assert.deepEqual(Array.from(rendered.customSections, (section) => section.id), ['custom-first', 'custom-late']);
  assert(!rendered.customSections.some((section) => section.id === 'custom-draft'));
}
...
assert.equal(visitorSectionsFor('en').customSections[0].displayTitle, 'First');
